...
from yugioh_reader.config import LOCKED_PROFILE, config
from yugioh_reader.openai_realtime import OpenaiRealtimeHandler
from yugioh_reader.headless_personality_ui import mount_personality_routes
...
class LocalStream:
    ...
    def _persist_personality(self, profile: Optional[str]) -> None:
        ...
        try:
            from yugioh_reader.config import set_custom_profile
    ...
    def _init_settings_ui_if_needed(self) -> None:
        ...
        def _ready() -> JSONResponse:
            try:
                mod = sys.modules.get("yugioh_reader.tools.core_tools")
        ...
        async def _validate_key(payload: ApiKeyPayload) -> JSONResponse:
            ...
            try:
                ...
                logger.error(f"API key validation failed with a system error: {type(e).__name__}: {e}")
                return JSONResponse({"valid": False, "error": f"validation_error: {str(e)}"}, status_code=500)
