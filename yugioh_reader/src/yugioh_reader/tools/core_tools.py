...
from yugioh_reader.config import DEFAULT_PROFILES_DIRECTORY as DEFAULT_PROFILES_PATH  # noqa: F401

# Import config to ensure .env is loaded before reading REACHY_MINI_CUSTOM_PROFILE
from yugioh_reader.config import config  # noqa: F401
from yugioh_reader.tools.tool_constants import SystemTool


if TYPE_CHECKING:
    from yugioh_reader.tools.background_tool_manager import BackgroundToolManager


logger = logging.getLogger(__name__)


DEFAULT_PROFILES_MODULE = "yugioh_reader.profiles"
...
def _load_profile_tools() -> None:
    ...
    for tool_name in tool_names:
        ...
        if not loaded:
            shared_module_path = f"yugioh_reader.tools.{tool_name}"
            try:
                source = _try_load_tool(
                    tool_name,
